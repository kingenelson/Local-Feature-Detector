...
def get_feat_vec(x,y,magnitudes, orientations,feature_width):
    ...
    fw = feature_width
    ori_win = orientations[int(y-fw/2):int(y+fw/2), int(x-fw/2):int(x+fw/2)]
    mag_win = magnitudes[int(y-fw/2):int(y+fw/2), int(x-fw/2):int(x+fw/2)]
    for i in range(4):
        for j in range(4):
            sub_ori = ori_win[int(i*fw/4):int((i+1)*fw/4), int(j*fw/4):int((j+1)*fw/ 4)]
            sub_mag = mag_win[int(i*fw/4):int((i+1)*fw/4), int(j*fw/4):int((j+1)*fw/ 4)]
            hist = np.asarray(np.histogram(sub_ori, bins=8, range=(-np.pi, np.pi), weights=sub_mag)[0])
            fv = np.concatenate((fv, hist), axis = 0)
...
def get_features(image, x, y, feature_width):
    ...
    magn, ornt = get_magnitudes_and_orientations(dx, dy)
    for i in range(x.shape[0]):
        fvs[i,:] = get_feat_vec(x[i], y[i], magn, ornt, feature_width)
