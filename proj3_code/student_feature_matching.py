...
def match_features(features1, features2, x1, y1, x2, y2):
    ...
    d = compute_feature_distances(features1,features2)
    m_idx = []
    for i in range(d.shape[0]):
        idx = np.argsort(d[i,:])[:2]
        if d[i,idx[0]]/d[i,idx[1]] < .8:
            m_idx.append((i,idx[0]))
    # matches = ndarray([[l[0], l[1]] for l in m_idx])
    matches = np.asarray(m_idx)
    confidences = None
